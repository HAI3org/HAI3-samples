...
import { EXECUTIVE_SUMMARY_SCREENSET_ID, SOFTWARE_SCREEN_ID } from '../../ids';
 ...
 */
export const SoftwareScreen: React.FC = () => {
  useScreenTranslations(EXECUTIVE_SUMMARY_SCREENSET_ID, SOFTWARE_SCREEN_ID, translations);
  const { t } = useTranslation();
  
  // Get mock data
  const data = executiveSummaryMockMap['GET /api/executive-summary']() as ExecutiveSummaryData;
  
  const tk = (key: string) => t(`screen.${EXECUTIVE_SUMMARY_SCREENSET_ID}.${SOFTWARE_SCREEN_ID}:${key}`);
...
SoftwareScreen.displayName = 'SoftwareScreen';
